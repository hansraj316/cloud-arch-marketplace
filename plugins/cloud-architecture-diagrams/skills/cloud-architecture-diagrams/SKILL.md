...
description: >-
  ...
  and GCP / Google) using official vendor icons, output as BOTH .excalidraw and
  .drawio with real icons embedded and clean labeled arrows. Use whenever
  someone wants to create, draw, recreate, or lay out an Azure, Microsoft, Power
  Platform, Dynamics 365, Copilot Studio, Fabric, Entra, AWS (EC2, S3, Lambda,
  VPC, ECS/EKS, RDS, DynamoDB), or GCP (Compute Engine, GKE, BigQuery, Cloud
  Run, Vertex AI) architecture/solution diagram — from a text description OR by
  uploading a sketch/screenshot to reproduce. Triggers: "architecture diagram",
  "Azure/AWS/GCP diagram", "solution architecture", "draw this with cloud
  icons", "recreate this diagram", "make a .drawio/excalidraw of this", or
  pasting a diagram image for an editable version. Prefer over the plain inline
  diagram tool when official cloud product icons are wanted, since that tool
  can't embed them.
---
